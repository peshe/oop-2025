...
public:
	input_filebuf_iterator(): file(nullptr) {}
	input_filebuf_iterator(std::fstream& file): file(&file)
	{
		readNext();
	}
	...
	input_filebuf_iterator& operator++() // prefix
	{
		readNext();
		return *this;
	}
	input_filebuf_iterator operator++(int dummy) // postfix
	{
		input_filebuf_iterator old = *this;
		++*this;
		return old;
	}

	bool operator==(const input_filebuf_iterator& rhs) const
	{
...
template<class T>
class output_filebuf_iterator
{
	std::fstream* file;
public:
	output_filebuf_iterator(std::fstream& file): file(&file) {}
	output_filebuf_iterator& operator=(const T& val)
	{
		file->write(reinterpret_cast<const char*>(&val), sizeof(val));
		return *this;
	}
	output_filebuf_iterator& operator*()
	{
		return *this;
	}
	output_filebuf_iterator& operator++()
	...
	output_filebuf_iterator operator++(int)
	{
		return *this;
	}
};
...
int main(int argc, char** argv)
{
	...
	merge(input_filebuf_iterator<std::uint32_t>(temp1), input_filebuf_iterator<std::uint32_t>(),
		  input_filebuf_iterator<std::uint32_t>(temp2), input_filebuf_iterator<std::uint32_t>(),
		  output_filebuf_iterator<std::uint32_t>(file), less);
