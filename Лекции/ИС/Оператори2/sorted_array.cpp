...
bool SortedArray::remove(const SAIterator& where)
{
    if (&where.owner != this) {
...
bool SortedArray::operator!()const
{
    return empty();
}

SortedArray::operator bool() const
{
    return !empty();
}
...
SortedArray::SAIterator operator-(const SortedArray::SAIterator& iter, int step)
{
    SortedArray::SAIterator old(iter);
    return old -= step;
}
