...
public:
    ...
    bool transfer(const char *sourceClientIdentificationNumber, const char *destinationClientIdentifinationNumber,
                  Money moneyToTransfer);

    bool convertClientMoney(const char *identificationNumber, Currency newCurrency);

    // ====== New stuff:
    Client &operator[](std::size_t index) const;
...
private:
    ...
    std::size_t clientsSize{0};
    std::size_t clientsCapacity{0};

    ConversionRate conversionRates[CONVERSION_RATES_SIZE];
};
